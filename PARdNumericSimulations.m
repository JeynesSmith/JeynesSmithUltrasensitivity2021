% This function is used to simulate the PAR-d system, and find steady
...
% W, Ws, and E2. The parameters are defined by a1, d1, ..., d3, k3.
...
eqns = zeros(7,1); % To start with we have six empty equations
% Using u = [W  Ws E1 E2 C1 C2 C3] 
% Using k = [a1,d1,k1,a2,d2,k2,a3,d3,k3]
%           [ 1  2  3  4  5  6  7  8  9
eqns(1) = k(2)*u(5) + k(6)*u(6) + k(8)*u(7) - k(1)*u(1)*u(3) - k(7)*u(1)*u(2);
eqns(2) = k(5)*u(6) + k(3)*u(5) + (k(8) + 2*k(9))*u(7) - k(4)*u(2)*u(4) - k(7)*u(1)*u(2);
eqns(3) = (k(2) + k(3))*u(5) - k(1)*u(1)*u(3);
eqns(4) = (k(5) + k(6))*u(6) - k(4)*u(2)*u(4);
eqns(5) = k(1)*u(1)*u(3) - (k(2) + k(3))*u(5);
eqns(6) = k(4)*u(2)*u(4) - (k(5) + k(6))*u(6);
eqns(7) = k(7)*u(1)*u(2) - (k(8) + k(9))*u(7);
end